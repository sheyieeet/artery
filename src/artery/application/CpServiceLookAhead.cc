...
#include "artery/application/CpServiceLookAhead.h"
...
Define_Module(CpServiceLookAhead);

CpServiceLookAhead::CpServiceLookAhead() : mGenCpmMin{100, SIMTIME_MS}, mGenCpmMax{1000, SIMTIME_MS}, mGenCpm(mGenCpmMax)
{
}

CpServiceLookAhead::~CpServiceLookAhead()
{
    if (findHost()) {
        try {
...
void CpServiceLookAhead::initialize()
{
...
void CpServiceLookAhead::indicate(const vanetza::btp::DataIndication& ind, std::unique_ptr<vanetza::UpPacket> packet)
{
                ...
                for (size_t i = 0; i < poc.perceivedObjects.list.count; ++i) {
                    ...
                    if (idRegistry) {
                        ...
                        if (identity && identity->host) {
                            ...
                            if (!isAlive) continue; // Skip processing because sender is dead
...
void CpServiceLookAhead::trigger()
{
...
void CpServiceLookAhead::checkTriggeringConditions(const SimTime& T_now)
{
...
void CpServiceLookAhead::sendCpm(const SimTime& T_now)
{
    ...
    auto cpm = createCollectivePerceptionMessageLookAhead(mVdpSnapshot, referenceTime);

    if (mVdpSnapshot.stationType == static_cast<int>(vanetza::geonet::StationType::RSU)) {
        addOriginatingRsuContainerLookAhead(cpm);
    } else {
        addOriginatingVehicleContainerLookAhead(cpm, mVdpSnapshot);
    }
    if (checkSensorInformationTrigger(T_now)) {
        addSensorInformationContainerLookAhead(cpm, mSensorSnapshot);
        mLastSensorInformationTimestamp = T_now;
    }
    if (checkPerceptionRegionTrigger()) {
        addPerceptionRegionContainerLookAhead(cpm);
    }
    bool includeObjects = checkPerceivedObjectTrigger(T_now);
    if (includeObjects) {
        addPerceivedObjectContainerLookAhead(cpm, mPerceivedObjectSnapshot, mSelectedCpmObjects, mLastCpmTimestamp);
    }
...
void CpServiceLookAhead::sendSelectedLink()
{
    omnetpp::SimTime t_now = simTime();
    ...
    std::vector<CpServiceLookAhead::PerceivedObjectSnapshot> selectedSnapshots;
    ...
    for (const auto& pair : mGlobalObstaclesList) {
        ...
        if (include) {
            CpServiceLookAhead::PerceivedObjectSnapshot snap;
    ...
    if (!selectedSnapshots.empty()) {
        ...
        addOriginatingRsuContainerLookAhead(responseCpm);
        addPerceivedObjectContainerLookAhead(responseCpm, selectedSnapshots, selectedIndices, mLastCpmTimestamp);
...
void CpServiceLookAhead::captureVdpSnapshot()
{
    if (mVehicleDataProvider) {
        ...
        if (geoPosition && !std::isnan(geoPosition->latitude.value()) && !std::isnan(geoPosition->longitude.value())) {
            double latDeg = geoPosition->latitude / vanetza::units::degree;
            double lonDeg = geoPosition->longitude / vanetza::units::degree;
            if (latDeg >= -90.0 && latDeg <= 90.0 && lonDeg >= -180.0 && lonDeg <= 180.0) {
                mVdpSnapshot.referenceLatitude = std::round(latDeg * 1e7);
                mVdpSnapshot.referenceLongitude = std::round(lonDeg * 1e7);
            } else {
    ...
    if (mVdpSnapshot.referenceLatitude < -900000000 || mVdpSnapshot.referenceLatitude > 900000000) {
        mVdpSnapshot.referenceLatitude = 900000001; // unavailable
    }
    if (mVdpSnapshot.referenceLongitude < -1800000000 || mVdpSnapshot.referenceLongitude > 1800000000) {
        mVdpSnapshot.referenceLongitude = 1800000001; // unavailable
    }
}

void CpServiceLookAhead::captureSensorSnapshot(const omnetpp::SimTime& T_now, const std::vector<Sensor*>& sensors)
{
...
CpServiceLookAhead::ObjectVdpSnapshotMap CpServiceLookAhead::captureObjectVdpSnapshot(const LocalEnvironmentModel::TrackedObjects& objects) const
{
...
void CpServiceLookAhead::capturePerceivedObjectSnapshot(
...
bool CpServiceLookAhead::checkSensorInformationTrigger(const SimTime& T_now)
{
...
bool CpServiceLookAhead::checkPerceptionRegionTrigger()
{
...
bool CpServiceLookAhead::checkPerceivedObjectTrigger(const SimTime& T_now)
{
    ...
    for (const auto idx : mSelectedCpmObjects) {
        ...
        if (po.haveHistory) {
            ...
            // Look ahead : next speed change
            double nextS = po.speedDiffMps + (po.speedDiffMps/mGenCpm.dbl()) * mGenCpm.dbl();
            if(nextS > mMinGroundSpeedChangeThreshold.value()){
                selected.push_back(idx);
                continue;
            }
...
double CpServiceLookAhead::calculateUtilityFunction(
...
void CpServiceLookAhead::sortPerceivedObjects()
{
...
SimTime CpServiceLookAhead::genCpmDcc()
{
...
void CpServiceLookAhead::handlePseudonymChange()
{
...
std::optional<uint16_t> CpServiceLookAhead::allocateCpmObjectId(const omnetpp::SimTime& T_now, uint32_t lemId, int64_t objectAgeMs)
{
...
std::optional<uint8_t> CpServiceLookAhead::allocateCpmSensorId(const omnetpp::SimTime& T_now, const int sensorId)
{
...
CpServiceLookAhead::PerceivedObjectType CpServiceLookAhead::toPOType(vanetza::geonet::StationType st)
{
    using vanetza::geonet::StationType;
    switch (st) {
...
Cpm createCollectivePerceptionMessageLookAhead(const CpServiceLookAhead::VdpSnapshot& vdp, uint64_t referenceTime)
{
    Cpm message;
...
void addOriginatingVehicleContainerLookAhead(Cpm& message, const CpServiceLookAhead::VdpSnapshot& vdp)
{
...
void addOriginatingRsuContainerLookAhead(Cpm& message)
{
    Vanetza_ITS2_WrappedCpmContainer_t* wcc = vanetza::asn1::allocate<Vanetza_ITS2_WrappedCpmContainer_t>();
...
void addSensorInformationContainerLookAhead(Cpm& message, const std::vector<CpServiceLookAhead::SensorSnapshot>& sensorSnapshot)
{
    Vanetza_ITS2_WrappedCpmContainer_t* wcc = vanetza::asn1::allocate<Vanetza_ITS2_WrappedCpmContainer_t>();
...
void addPerceptionRegionContainerLookAhead(Cpm& message)
{
    // Vanetza_ITS2_WrappedCpmContainer_t* wcc = vanetza::asn1::allocate<Vanetza_ITS2_WrappedCpmContainer_t>();
...
void addPerceivedObjectContainerLookAhead(
    Cpm& message, const std::vector<CpServiceLookAhead::PerceivedObjectSnapshot>& perceivedObjectSnapshot, const std::vector<std::size_t>& selectedObjects,
...
void CpServiceLookAhead::finish()
{
    if (mCpmTimer) {
...
void CpServiceLookAhead::handleMessage(omnetpp::cMessage* msg)
{
    if (msg == mCpmTimer) {
        sendSelectedLink();
        scheduleAt(simTime() + 0.1, mCpmTimer);
...
void CpServiceLookAhead::receiveSignal(omnetpp::cComponent* source, omnetpp::simsignal_t signal, double value, omnetpp::cObject* details)
{
...
void CpServiceLookAhead::writeMetricToCsv(const std::string& metricType, double value, long objectId)
{
    ...
    if (!outfile.is_open()) {
        std::string filename = "cpm_metrics_lookahead.csv";
...
void CpServiceLookAhead::updatePerceptionRate()
{
    ...
    auto identity = getFacilities().get_const_ptr<Identity>();
    std::string egoTraciId = identity ? identity->traci : "";

    auto allObjects = globalEnv->getAllObjects();
    for (const auto& obj : allObjects) {
        if (obj->getExternalId() == egoTraciId) continue;
